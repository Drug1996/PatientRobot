...
// Version     :
// Copyright   : 2019
// Description : robot patient in C++
//============================================================================
...
int PrepareDemo(unsigned int* p_pErrorCode)
{
	...
	if(VCS_GetFaultState(g_pKeyHandle, g_subNodeId, &oIsFault, p_pErrorCode ) == 0)
	{
		LogError("VCS_GetFaultState", lResult, *p_pErrorCode);
		lResult = MMC_FAILED;
	}
	...
	if(lResult==0)
	{
		if(oIsFault)
		{
			...
			if(VCS_ClearFault(g_pKeyHandle, g_subNodeId, p_pErrorCode) == 0)
			{
				LogError("VCS_ClearFault", lResult, *p_pErrorCode);
				lResult = MMC_FAILED;
			}
		...
		if(lResult==0)
		{
			...
			if(VCS_GetEnableState(g_pKeyHandle, g_subNodeId, &oIsEnabled, p_pErrorCode) == 0)
			{
				LogError("VCS_GetEnableState", lResult, *p_pErrorCode);
				lResult = MMC_FAILED;
			}
			...
			if(lResult==0)
			{
				if(!oIsEnabled)
				{
					if(VCS_SetEnableState(g_pKeyHandle, g_subNodeId, p_pErrorCode) == 0)
					{
						LogError("VCS_SetEnableState", lResult, *p_pErrorCode);
						lResult = MMC_FAILED;
					}
...
int Demo(unsigned int* p_pErrorCode)
{
	...
	{
		lResult = DemoProfileVelocityMode(g_pKeyHandle, g_subNodeId, lErrorCode);
...
int PrintDeviceVersion()
{
	...
	if(VCS_GetVersion(g_pKeyHandle, g_subNodeId, &usHardwareVersion, &usSoftwareVersion, &usApplicationNumber, &usApplicationVersion, &ulErrorCode))
	{
		printf("%s Hardware Version    = 0x%04x\n      Software Version    = 0x%04x\n      Application Number  = 0x%04x\n      Application Version = 0x%04x\n",
				g_deviceName.c_str(), usHardwareVersion, usSoftwareVersion, usApplicationNumber, usApplicationVersion);
		lResult = MMC_SUCCESS;
